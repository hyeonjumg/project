...
<body>
  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    <ul class="nav navbar-nav">
 		...
 		<div class="container">
 		<br><br><br>
 		<h3 style="text-align: left;"><I><a href="custom.jsp">고객센터[Q & A]</a></I></h3><BR><BR>
 			...
 			<div class="row">
 					<table class="table table-striped" style="text-align: center;  border: 1px solid #dddddd">
 						...
 						<%
 							...
 							ArrayList<Custom> list = customDAO.getList(pageNumber);
 							for(int i = 0; i < list.size(); i++) {
 								
 							
 						%>
 							<tr>
 								<td><%= list.get(i).getCustomID() %></td>
 								<td><a href="customview.jsp?customID=<%= list.get(i).getCustomID() %>"><%= list.get(i).getCustomTitle() %></a></td>
 								<td><%= list.get(i).getUserID() %></td>
 								<td><%= list.get(i).getCustomDate() %></td>
 					...
 					<%
 						if(pageNumber != 1) {
 					%>
 						<a href="custom.jsp?pageNumber=<%=pageNumber -1 %>" class="btn btn-success btn-arraw-left">이전</a>
 					<%
 						} if(customDAO.nextPage(pageNumber + 1)) {
 					%>
 						<a href="custom.jsp?pageNumber=<%=pageNumber +1 %>" class="btn btn-success btn-arraw-left">다음</a>
 					<%
 						}
 					%>
